...
{
public:
    DSPADPCMSTMImportFileHandle(wxFile *file, const wxString& filename);
    ~DSPADPCMSTMImportFileHandle();
...
{
public:
    DSPADPCMGCUBImportFileHandle(wxFile *file, const wxString& filename);
    ~DSPADPCMGCUBImportFileHandle();
...
ImportFileHandle *DSPADPCMImportPlugin::Open(const wxString& filename)
{
    ...
    {
        DSPADPCMGCUBImportFileHandle* fh =
        new DSPADPCMGCUBImportFileHandle(file, filename);
        if (!fh->valid)
        {
            delete fh;
            return NULL;
...
DSPADPCMSTMImportFileHandle::DSPADPCMSTMImportFileHandle(wxFile *file, const wxString& filename)
    :  DSPADPCMBaseImportFileHandle(filename)
{
    wxASSERT(file);
    mFile = file;
...
int DSPADPCMSTMImportFileHandle::Import(TrackFactory *trackFactory, TrackHolders& outTracks, Tags *tags)
{
    CreateProgress();
...
DSPADPCMGCUBImportFileHandle::DSPADPCMGCUBImportFileHandle(wxFile *file, const wxString& filename)
: DSPADPCMBaseImportFileHandle(filename)
{
    wxASSERT(file);
    mFile = file;
...
int DSPADPCMGCUBImportFileHandle::Import(TrackFactory *trackFactory,
                                         TrackHolders &outTracks,
                                         Tags *tags)
{
    CreateProgress();
...
DSPADPCMGCUBImportFileHandle::~DSPADPCMGCUBImportFileHandle()
{
    if (mFile)
    {
        if (mFile->IsOpened())
